...
RUN go build -v -modcacherw -o /output/$TARGETOS/$TARGETARCH/revocation-grpc-plugin-server-go .
...
# Copy build
COPY --from=builder /output/$TARGETOS/$TARGETARCH/revocation-grpc-plugin-server-go revocation-grpc-plugin-server-go
